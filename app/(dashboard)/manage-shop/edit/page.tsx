...
const mockShopItem = {
  id: 1,
  itemName: "Health Potion (L)",
  description: "A large health potion that restores 500 HP.",
  price: 500,
  currencyType: "Gold",
  stock: -1,
  isActive: true,
...
export default function EditShopItemPage() {
  ...
  useEffect(() => {
    if (itemId) {
      setFormData({
        itemName: mockShopItem.itemName,
        description: mockShopItem.description,
        price: mockShopItem.price,
        currencyType: mockShopItem.currencyType,
        stock: mockShopItem.stock,
        isActive: mockShopItem.isActive,
      });
    }
  }, [itemId]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Updating shop item:", formData);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-white/50 text-sm">Update shop item details (ID: {itemId})</p>
        </div>
      </div>
